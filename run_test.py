...
parser.add_argument('--check',
                    help='this optionchecks if freesurfer analysis has been done. if yes, it will skip the process otherwise perform recon-all')
parser.add_argument('--ctp',
                    help='this option predicts pain sensitivity using participant-level cortical thickness',
                    choices = ['true', 'false'],
                    default = 'false')
...
# compute cortical thickness based pain sensitivity using the ctp-signature stored in ./ml_models/ctp_signature_model.sav
# few imports 
import joblib

if args.ctp == 'true':
    print('now, ctp-score is calculated..')
    cmd = 'ls /output '
    os.system(cmd)
    for ids in args.participant_label:
        id_assigned = ('sub-' + ids)

        hemis = ['lh', 'rh']
        for hemi_name in hemis:
            cmd = ('python2 `which aparcstats2table` ' + '--subjects ' + args.output_dir + '/' + id_assigned +  \
                ' --hemi ' + hemi_name + ' --meas thickness --tablefile ' + args.output_dir + '/' + \
                id_assigned + '_' + hemi_name + '_thickness.csv')
            os.system(cmd)
        print(cmd)

        df_thickness_lf = pd.read_csv((args.output_dir + '/' + id_assigned + '_' + hemis[0] + '_thickness.csv'), sep='\t')
        df_thickness_rf = pd.read_csv((args.output_dir + '/' + id_assigned + '_' + hemis[1] + '_thickness.csv'), sep='\t')
        df_thickness_combined = pd.concat([df_thickness_lf, df_thickness_rf], axis=1)
        df_thickness = df_thickness_combined.filter(like='_thickness')
        df_thickness.drop(['lh_MeanThickness_thickness', 'rh_MeanThickness_thickness'], axis=1, inplace=True)

        X= df_thickness.div(df_thickness.mean(axis=1), axis=0)

        model = joblib.load('./ml_models/ctp_signature_model.sav')

        ctp_score = pd.DataFrame(model.predict(X))
        ctp_score.columns=['ctp_score']
        ctp_score.rename(index={0:id_assigned}, inplace=True)
        ctp_score.to_csv(args.output_dir + '/' + id_assigned + '_ctp_score.csv')

        cmd = ('rm ' + args.output_dir + '/' + id_assigned + '*thickness.csv')
        os.system(cmd)
